...
SOURCE="${1:-/tmp/tcds-eif-milestone-1b5-green-tier1-v0.9.0}"
...
[[ -f "$SOURCE/release/manifest-v0.9.0.json" ]] ||
  fail "Extracted 1B.5 GT1 package not found at: $SOURCE"
[[ "$(<"$TARGET/VERSION")" == "0.7.0" ]] ||
  fail "Milestone 1B.4 version 0.7.0 is required."
compgen -G \
  "$TARGET/manifests/installations/milestone-1b4-green-tier1-*.json" \
  >/dev/null ||
  fail "A successful Milestone 1B.4 GT1 manifest is required."
[[ -x "$TARGET/bin/eif-exec-v2" ]] ||
  fail "Milestone 1B.4 controlled execution engine is missing."
...
[[ "$(sha256sum "$SOURCE/release/manifest-v0.9.0.json" | awk '{print $1}')" == \
  "ddd299c4cba78e33fb3491b53aaea844184e84cf75c4c59da9a01663bef90ca9" ]] ||
  fail "Release manifest checksum mismatch."
[[ "$(sha256sum "$SOURCE/release/verify_release_v0.9.0.py" | awk '{print $1}')" == \
  "54cef613cf8e8f708ce6b46dbc96b4ab06388d8d6e1c32c0519e14d203ebf270" ]] ||
  fail "Release verifier checksum mismatch."

python3 "$SOURCE/release/verify_release_v0.9.0.py" "$SOURCE"

PARENT="$(dirname "$TARGET")"
BACKUP_ROOT="$PARENT/eif-recovery-backups/1b5-$TIMESTAMP"
ORIGINAL="$BACKUP_ROOT/framework-pre-1b5"
FAILED="$BACKUP_ROOT/framework-failed-1b5"
STAGE_CONTAINER="$(mktemp -d "$PARENT/.eif-1b5-stage.XXXXXX")"
STAGE="$STAGE_CONTAINER/framework"
LOCK_FILE="$PARENT/.eif-milestone-1b5-recovery.lock"
SWAPPED=0
...
rollback_on_error() {
  ...
  if [[ "$SWAPPED" -eq 1 && -d "$ORIGINAL" ]]; then
    printf 'Live validation failed; restoring the pre-1B.5 framework.\n' >&2
    if [[ -d "$TARGET" ]]; then
      mv "$TARGET" "$FAILED"
    fi
    mv "$ORIGINAL" "$TARGET"
...
# Copy only files authenticated by the complete v0.9.0 release manifest.
...
manifest_path = source / "release" / "manifest-v0.9.0.json"
...
# Keep the complete manifest available for audit; it authenticates itself
# through the wrapper's pinned checksum above.
audit_manifest = target / "release" / manifest_path.name
shutil.copyfile(manifest_path, audit_manifest)
os.chmod(audit_manifest, 0o640)
PY

install -d -m 0750 \
  "$STAGE/state/checkpoints" \
  "$STAGE/state/checkpoints-v2" \
  "$STAGE/state/components" \
  "$STAGE/state/database" \
  "$STAGE/state/exports" \
  "$STAGE/state/locks" \
  "$STAGE/state/recovery" \
  "$STAGE/state/transactions" \
  "$STAGE/state/transactions/idempotency"
...
MANIFEST="$TARGET/manifests/installations/milestone-1b5-green-tier1-$TIMESTAMP.json"
...
payload = {
    ...
    "status": "SUCCESS",
    "validated": True,
    "baseStateVersion": "0.8.0",
    "stateEngineVersion": "0.9.0",
    "backup": backup_path,
...
printf '1B.5 Green Tier 1 recovery completed.\n'
printf 'Manifest: %s\n' "$MANIFEST"
