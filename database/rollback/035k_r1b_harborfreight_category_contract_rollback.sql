...
BEGIN
  ...
  WHERE a.adapter_code = 'harborfreight_brightdata_category'
    AND a.adapter_version = '1';

  IF NOT FOUND THEN
    RAISE NOTICE 'Harbor Freight category-URL authority is not installed';
    RETURN;
  END IF;

  IF adapter_row.scraper_asset_id <>
     'e9e1fc56-3b92-4720-9266-228e8ebac20d'::uuid THEN
    RAISE EXCEPTION 'Harbor Freight adapter asset changed; rollback refused';
  END IF;
  ...
  VALUES (
    ...
    'user', 'tictac', 'Tictac',
    'r1b-harborfreight-url-rollback', 'r1b-v4-harborfreight-1',
    current_setting('tcds.code_version'),
    'r1b-v4.0.0',
    'retail.retail_scraper_contracts',
    'R1B:HARBORFREIGHT:CATEGORY:ROLLBACK:' || correlation_id
  )
  ...
  IF adapter_row.contract_status NOT IN ('blocked', 'retired') THEN
    PERFORM retail.r1b_transition_scraper_contract(
      ...
      correlation_id,
      'tictac',
      'Harbor Freight category-URL authority production rollback'
    );
  END IF;
  ...
  RAISE NOTICE
    'Harbor Freight adapter % and contract % blocked',
