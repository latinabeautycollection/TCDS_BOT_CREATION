...
BEGIN
  ...
  IF v_definition NOT LIKE
     '%v_observation_doc - ''collection_run_id''%' THEN
    RAISE EXCEPTION
      'Evaluator still hashes collection_run_id';
  END IF;

  v_a := retail.r1e_sha256_jsonb(
    '{"collection_run_id":"00000000-0000-0000-0000-000000000001",
       "platform_product_key":"93138477"}'::jsonb
    - 'collection_run_id'
  );

  v_b := retail.r1e_sha256_jsonb(
    '{"collection_run_id":"00000000-0000-0000-0000-000000000002",
       "platform_product_key":"93138477"}'::jsonb
    - 'collection_run_id'
  );
  ...
  IF v_a IS DISTINCT FROM v_b THEN
    RAISE EXCEPTION
      'Collection-run provenance still changes observation identity';
  END IF;
END
$regression$;
