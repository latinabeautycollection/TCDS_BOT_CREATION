...
CREATE OR REPLACE FUNCTION retail.r1e_returned_variant_attributes(p_returned jsonb)
 RETURNS jsonb
 LANGUAGE sql
 IMMUTABLE
AS $function$
    ...
    || jsonb_build_object(
      ...
      'normalized_model_token',
        NULLIF(
          ...
          p_returned->>'platform_product_key'
        ),
      'generation',
        COALESCE(
...
DECLARE
  ...
  v_real jsonb;
BEGIN
  v_false := retail.r1e_returned_variant_attributes(
    '{
      "platform_product_key":"93138477",
      "model_number":"93138477",
      "normalized_identity":{
        "normalized_model_token":"93138477"
      }
    }'::jsonb
  );

  IF v_false ? 'normalized_model_token' THEN
    RAISE EXCEPTION
      'Retailer product key remained a normalized model token';
  END IF;

  v_real := retail.r1e_returned_variant_attributes(
    '{
      "platform_product_key":"93138477",
  ...
  IF v_real->>'normalized_model_token' <> 'iPhone 15 Pro' THEN
    RAISE EXCEPTION 'Genuine model number was not preserved';
  END IF;
END
$regression$;
