...
BEGIN
  ...
  IF position(
    'v_cost_basis:=a.cost_basis;' IN v_definition
  ) = 0 THEN
    RAISE EXCEPTION
      'R1F ingest does not use authoritative attempt cost_basis';
  END IF;

  IF position(
    'WHEN a.actual_cost_usd IS NOT NULL THEN ''actual''' IN v_definition
  ) > 0 THEN
    RAISE EXCEPTION
      'R1F ingest still infers actual basis from a non-null amount';
  END IF;
END
$regression$;
