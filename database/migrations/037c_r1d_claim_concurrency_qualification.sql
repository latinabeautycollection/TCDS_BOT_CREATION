...
BEGIN
  ...
  LOOP
    ...
    FROM retail.r1d_dispatch_jobs running
    WHERE running.platform_id=j.platform_id
      AND running.status IN('leased','dispatching')
    ...
    SELECT count(*)::int INTO v_running
    FROM retail.r1d_dispatch_jobs running
    WHERE running.dispatch_binding_id=j.dispatch_binding_id
      AND running.status IN('leased','dispatching')
      AND running.lease_expires_at>now();
