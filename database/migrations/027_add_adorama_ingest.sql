...
VALUES (
  'adorama', 'Adorama', 'https://www.adorama.com',
  'authorized_scraper', 'active', 36, true, true,
  'https://www.adorama.com/robots.txt',
  'https://www.adorama.com/help/termsAndConditions',
  'Bright Data Adorama PDP dataset with category discovery through tcds_web_unlocker.'
)
...
INSERT INTO retail.platform_collection_configs (
  platform_id, config_name, is_active, collection_strategy,
  category_seed_json, search_seed_json, request_policy_json,
  parser_policy_json, evidence_policy_json
)
SELECT
  p.id,
  ...
  jsonb_build_array(
    'https://www.adorama.com/l/Audio',
    'https://www.adorama.com/l/Home-Electronics',
    'https://www.adorama.com/l/Home-Electronics/Wearable-Tech',
    'https://www.adorama.com/l/Audio/Microphones/Wireless-Microphones'
  ),
  '[]'::jsonb,
  jsonb_build_object(
    'dataset_id', 'gd_mlj9zzkr17l7vh9pno',
    'web_unlocker_zone', 'tcds_web_unlocker',
    'limit', 50,
    'detail_limit_per_input', 1,
    'provider_error_retry_limit', 0
  ),
  jsonb_build_object('parser_version', 'brightdata_adorama_v1'),
  jsonb_build_object('store_raw', true, 'source_platform', 'adorama')
FROM retail.retail_platforms p
WHERE p.platform_code = 'adorama'
...
CREATE TABLE IF NOT EXISTS retail.adorama_product_parsed (
  ...
  source_platform text NOT NULL DEFAULT 'adorama'
    CHECK (source_platform = 'adorama'),
  ...
  source_dataset text NOT NULL DEFAULT 'gd_mlj9zzkr17l7vh9pno',
  parser_version text NOT NULL DEFAULT 'brightdata_adorama_v1',
...
CREATE INDEX IF NOT EXISTS ix_adorama_item_latest
  ON retail.adorama_product_parsed(item_id, parsed_at DESC);
CREATE INDEX IF NOT EXISTS ix_adorama_variant_latest
  ON retail.adorama_product_parsed(variant_id, parsed_at DESC)
  WHERE variant_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_adorama_mpn
  ON retail.adorama_product_parsed(mpn) WHERE mpn IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_adorama_price_availability
  ON retail.adorama_product_parsed(effective_price, availability);
CREATE INDEX IF NOT EXISTS ix_adorama_payload_gin
  ON retail.adorama_product_parsed USING gin(parsed_payload);

CREATE OR REPLACE FUNCTION retail.ingest_brightdata_adorama_product(
...
BEGIN
  ...
  WHERE platform_code = 'adorama';

  IF v_platform_id IS NULL THEN
    RAISE EXCEPTION 'Adorama platform is not registered';
  END IF;
  ...
  IF v_raw_platform_id <> v_platform_id THEN
    RAISE EXCEPTION 'Raw capture % is not an Adorama capture', p_raw_capture_id;
  END IF;

  v_url := nullif(trim(coalesce(p_payload->>'url', p_payload#>>'{input,url}')), '');
  v_item_id := nullif(trim(coalesce(
    p_payload->>'item_id', p_payload->>'product_id',
    substring(coalesce(v_url, '') from '/p/([^/?#]+)')
  )), '');
  v_variant_id := nullif(trim(coalesce(p_payload->>'variant_id', p_payload->>'sku')), '');
  ...
  IF v_item_id IS NULL THEN
    RAISE EXCEPTION 'Missing Adorama item_id/url';
  END IF;
  IF v_url IS NULL THEN
    RAISE EXCEPTION 'Missing Adorama product URL for item %', v_item_id;
  END IF;
  IF v_title IS NULL THEN
    RAISE EXCEPTION 'Missing Adorama title for item %', v_item_id;
  END IF;

  v_payload_hash := encode(digest(p_payload::text, 'sha256'), 'hex');

  INSERT INTO retail.adorama_product_parsed (
    raw_capture_id, collection_run_id, platform_id,
    item_id, variant_id, group_id, url, title, description, brand, mpn, gtin, upc,
    ...
    listing_has_variations, variant_attributes, variants, reviews,
  ...
  VALUES (
    ...
    v_item_id, v_variant_id, p_payload->>'group_id', v_url, v_title,
    p_payload->>'description', p_payload->>'brand', p_payload->>'mpn',
    p_payload->>'gtin', coalesce(p_payload->>'upc', p_payload->>'gtin'),
...
CREATE OR REPLACE FUNCTION retail.promote_adorama_parsed_product(
  p_adorama_parsed_id uuid
)
RETURNS uuid
LANGUAGE plpgsql
AS $$
DECLARE
  v_parsed retail.adorama_product_parsed%rowtype;
...
BEGIN
  ...
  FROM retail.adorama_product_parsed
  WHERE id = p_adorama_parsed_id;

  IF v_parsed.id IS NULL THEN
    RAISE EXCEPTION 'Adorama parsed product not found: %', p_adorama_parsed_id;
  END IF;
  IF v_parsed.effective_price IS NULL THEN
    RAISE EXCEPTION 'Adorama product % has no effective price', v_parsed.item_id;
  END IF;

  v_product_key := coalesce(nullif(v_parsed.variant_id, ''), v_parsed.item_id);
  v_availability := CASE
  ...
  VALUES (
    v_parsed.platform_id, v_product_key, v_parsed.url, v_parsed.title,
    v_parsed.brand, v_parsed.mpn, coalesce(v_parsed.upc, v_parsed.gtin),
    v_product_key, v_parsed.product_category, v_parsed.image_url,
  ...
  v_offer_snapshot_id := retail.record_offer_snapshot(
    v_retail_product_id, v_parsed.effective_price, v_availability,
    NULL, NULL, NULL, v_parsed.url, v_parsed.raw_capture_id,
    jsonb_build_object(
      'source', 'brightdata',
      'retailer', 'adorama',
      ...
      'variant_id', v_parsed.variant_id,
  ...
  UPDATE retail.adorama_product_parsed
