  ...
  LOOP
    ...
    WHERE platform.platform_code = definition.platform_code
      ...
      AND asset.git_commit_sha =
          'c1fb0ed8865ac4f7eaacdbd28b3db54bf3c473b9';

    SELECT prior_contract.id
    INTO STRICT prior_contract_id
    FROM retail.retail_search_adapters prior_adapter
    JOIN retail.retail_scraper_contracts prior_contract
      ON prior_contract.id = prior_adapter.scraper_contract_id
     AND prior_contract.scraper_asset_id =
         prior_adapter.scraper_asset_id
    JOIN retail.retail_scraper_assets prior_asset
      ON prior_asset.id = prior_adapter.scraper_asset_id
    WHERE prior_adapter.platform_id = asset_record.platform_id
      AND prior_adapter.adapter_code = definition.adapter_code
      AND prior_adapter.adapter_version = '1'
      AND prior_adapter.certification_status = 'suspended'
      AND prior_contract.certification_status = 'blocked'
      AND prior_asset.git_commit_sha =
          '0d97399e2b3a6fa4b1423168cf59780af1c0bb0c'
      AND prior_asset.id <> asset_record.asset_id;
    ...
    IF FOUND THEN
      RAISE EXCEPTION
        'Adapter already exists: % version 2',
    ...
    VALUES (
      ...
      'user',
      'tictac',
      'Tictac R1B Contract V2 Bootstrap',
      'r1b-amazon-target-contract-v2-bootstrap',
      'r1b-v4-2',
    ...
    evidence_json := jsonb_build_object(
      ...
      'verified_interface', jsonb_build_object(
        ...
        'postal_transport',
          CASE
            ...
            ELSE 'zero_national_one_broadcast_or_positional_zip_list'
          END,
      ...
      'certification_limitations', jsonb_build_array(
        ...
        'Requested ZIP provenance is retained in run and capture metadata',
        CASE
          WHEN definition.platform_code = 'target'
            THEN 'ZIP mapping supports zero ZIPs, one broadcast ZIP, or one ZIP per keyword'
