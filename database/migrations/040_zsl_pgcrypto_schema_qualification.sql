...
BEGIN
  IF to_regprocedure('extensions.digest(bytea,text)') IS NULL
    ...
    RAISE EXCEPTION
      'ZSL requires pgcrypto digest functions in extensions schema';
  END IF;
END
$preflight$;
...
sha:=encode(extensions.digest(convert_to(jsonb_build_object('ingest_run_id',p_run,'event_type',p_type,'actor',p_actor,'event_data',p_data)::text,'UTF8'),'sha256'),'hex');
...
BEGIN
 ...
 evsha:=encode(extensions.digest(convert_to(jsonb_build_object('eventType','ACTIVATED','buildRunId',p_build,'certificationRunId',p_cert,'runtimeCheckRunId',c.runtime_check_run_id,'actor',p_actor,'evidence',p_evidence)::text,'UTF8'),'sha256'),'hex');
...
BEGIN
 ...
 evsha:=encode(extensions.digest(convert_to(jsonb_build_object('eventType','REVOKED','buildRunId',s.build_run_id,'certificationRunId',s.certification_run_id,'actor',p_actor,'evidence',p_evidence)::text,'UTF8'),'sha256'),'hex');
...
BEGIN
  WITH signatures(signature) AS (
    VALUES
      ('zsl.append_audit_event(text,text,jsonb)'::regprocedure),
      ('zsl.append_ingest_audit_event(uuid,text,text,jsonb)'::regprocedure),
      ('zsl.zsl2_sha256_jsonb(jsonb)'::regprocedure),
      ('zsl.zsl3_activate(uuid,uuid,text,jsonb)'::regprocedure),
      ('zsl.zsl3_revoke_active(text,jsonb)'::regprocedure),
      ('zsl.zsl4_activate(uuid,uuid,text,jsonb)'::regprocedure),
      ('zsl.zsl4_finalize_runtime_check_run(uuid,text,jsonb)'::regprocedure),
      ('zsl.zsl4_revoke_active(text,jsonb)'::regprocedure)
  ),
  definitions AS (
    SELECT pg_get_functiondef(signature) AS definition
    FROM signatures
  )
  SELECT
    count(*),
    count(*) FILTER (
      WHERE definition LIKE '%extensions.digest(%'
    )
  INTO v_total, v_qualified
  FROM definitions;

  IF v_total <> 8
     OR v_qualified <> 8 THEN
    RAISE EXCEPTION
      'ZSL pgcrypto qualification regression: total %, qualified %',
      v_total,
      v_qualified;
  END IF;
END
