...
import type { DellRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-dell-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='dell'`);
    if (!p.rowCount) throw new Error("Dell platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'dell_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, { dataset_id: config.DELL_DATASET_ID, seed_urls: urls }]
  );
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,next_retry_at)
     VALUES($1,$2,'dell',$3,$4,$5,$6,$7,now()+interval '15 minutes')
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.dell_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
...
function isTargetScope(r:DellRecord): boolean {
  const text=[r.title,r.product_category,...r.category_tree.map(x=>x.name)].filter(Boolean).join(' ').toLowerCase();
  return /(dell|alienware|inspiron|latitude|xps|precision|optiplex|vostro|computer|laptop|desktop|workstation|monitor|dock|docking|keyboard|mouse|headset|webcam|printer|storage|ssd|hard drive|accessor|gaming|server)/i.test(text);
}

export async function ingestRecord(platformId:string,runId:string,r:DellRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    if (!scopeMatch) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'DELL_COLLECTION_SCOPE_MISMATCH','high','Dell PC/laptop/accessories deal seed returned an unrelated catalog product',$4)`,
    ...
    if (priceInversion) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'DELL_PRICE_FIELD_INVERSION','high','Dell sale_price exceeded regular price; lowest valid observed price selected',$4)`,
    ...
    if (regular !== null && sale !== null && Math.abs(regular - sale) < 0.005) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'DELL_DEAL_PAGE_NO_DISCOUNT','medium','Dell deal-page result has no observed price markdown; preserve evidence but do not assume discount qualification',$4)`,
        [platformId,productId,runId,{item_id:r.item_id,url:r.url,regular_price:regular,sale_price:sale,raw_capture_id:rawId}]
      );
    }
    ...
    if (effective !== null) {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,{source:'dell',effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion}]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,{dell_parsed_id:parsedId,regular_price:regular,sale_price:sale,effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion}]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,{dell_parsed_id:parsedId,collection_scope_match:scopeMatch,price_field_inversion:priceInversion}]
      );
    } else {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'DELL_PRICE_MISSING','medium','Dell dataset returned no usable price; product preserved but offer promotion withheld',$4)`,
    ...
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.dell_product_images(dell_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.dell_product_categories(dell_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
