...
import { OfficeDepotRecordSchema } from "./types.js";
import { platformAndConfig, createRun, finishRun, deadLetter, ingestRecord, pool } from "./db.js";
import { isOfficeDepotUrl, sha256, stableJson } from "./util.js";
import { unlockOfficeDepotPage } from "./unlocker.js";
import { discoverOfficeDepotProducts, findUnreturnedOfficeDepotInputs } from "./discovery.js";
...
async function main() {
  const badUrl = config.seedUrls.find(url => !isOfficeDepotUrl(url));
  if (badUrl) throw new Error(`Rejected non-Office Depot seed URL: ${badUrl}`);

  const discovered = await discoverOfficeDepotProducts(
    config.seedUrls,
    async url => (await unlockOfficeDepotPage(url)).body,
    config.OFFICE_DEPOT_LIMIT_PER_INPUT
  );
  const urls = discovered.map(product => product.url);
  const { platformId, configId } = await platformAndConfig();
  const runKey = `officedepot:${new Date().toISOString()}:${sha256(urls.join("|")).slice(0, 12)}`;
  const stats = { collected: 0, failed: 0, skipped: 0 };
  let runId = "";

  try {
    runId = await createRun(platformId, configId, runKey, urls);
    log("info", "officedepot_run_started", { runId, seeds: config.seedUrls, discovered: urls.length });
    ...
    if (!rows.length) throw new Error("OFFICE_DEPOT_SNAPSHOT_EMPTY");

    const unreturned = findUnreturnedOfficeDepotInputs(urls, rows);
    stats.skipped += unreturned.length;
    if (unreturned.length) {
      log("warn", "officedepot_provider_inputs_unreturned", { runId, snapshotId, count: unreturned.length, urls: unreturned });
    }
    ...
    await mapLimit(rows, config.INGEST_CONCURRENCY, async raw => {
      ...
      const parsed = OfficeDepotRecordSchema.safeParse(raw);
      if (!parsed.success) {
        stats.failed++;
    ...
    log("info", "officedepot_run_completed", { runId, ...stats, status });
  } catch (error) {
    if (runId) await finishRun(runId, "failed", stats, String(error));
    log("error", "officedepot_run_failed", { runId, error: String(error), ...stats });
    throw error;
  } finally {
    await pool.end();
  }
