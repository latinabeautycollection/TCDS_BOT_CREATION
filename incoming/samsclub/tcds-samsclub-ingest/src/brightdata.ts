...
export async function triggerDataset(urls:string[]):Promise<string>{
 const endpoint=`https://api.brightdata.com/datasets/v3/trigger?dataset_id=${encodeURIComponent(config.SAMSCLUB_DATASET_ID)}&notify=false&include_errors=true`;
 const r=await fetchWithRetry(endpoint,{method:'POST',headers,body:JSON.stringify({input:urls.map(url=>({url})),limit_per_input:config.SAMSCLUB_DETAIL_LIMIT_PER_INPUT})});
