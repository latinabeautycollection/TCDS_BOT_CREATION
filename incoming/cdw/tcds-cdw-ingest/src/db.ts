...
import type { CDWRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-cdw-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='cdw'`);
    if (!p.rowCount) throw new Error("CDW platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'cdw_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, pgJson({ dataset_id: config.CDW_DATASET_ID, seed_urls: urls })]
  );
  return r.rows[0].id as string;
}

export async function finishRun(id:string,status:'completed'|'failed'|'partial',stats:{collected:number;failed:number;skipped:number},reason?:string) {
  await pool.query(
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,next_retry_at)
     VALUES($1,$2,'cdw',$3,$4,$5,$6,$7,now()+interval '15 minutes')
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.cdw_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
...
function isTargetScope(r:CDWRecord): boolean {
  const text=[r.title,r.product_category,...r.category_tree.map(x=>x.name)].filter(Boolean).join(' ').toLowerCase();
  return /(cell phone|smartphone|iphone|android|mobile phone|phone accessory|smartwatch|smart watch|wearable|apple watch|galaxy watch|pixel watch|video conferenc|conference system|webcam|camera|headset|speakerphone|jabra|poly|logitech|yealink|teams room|zoom room)/i.test(text);
}

export async function ingestRecord(platformId:string,runId:string,r:CDWRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    const product=await c.query(
      ...
      [platformId,r.item_id,r.url,r.title,r.brand,r.mpn,r.variant_id,r.product_category,r.image_url,pgJson({gtin:r.gtin,group_id:r.group_id,availability_date:r.availability_date})]
    );
    ...
    if (!scopeMatch) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'CDW_COLLECTION_SCOPE_MISMATCH','high','CDW phone and smartwatch seed returned a product outside the governed mobile, wearable, or conferencing scope',$4)`,
    ...
    if (priceInversion) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'CDW_PRICE_FIELD_INVERSION','high','CDW sale_price exceeded regular price; lowest valid observed price selected',$4)`,
    ...
    if (regular !== null && sale !== null && Math.abs(regular - sale) < 0.005) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'CDW_CATEGORY_RESULT_NO_DISCOUNT','medium','CDW category result has equal regular and sale prices; preserve evidence but do not assume discount qualification',$4)`,
    ...
    if (effective !== null) {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,pgJson({source:'cdw',effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,pgJson({cdw_parsed_id:parsedId,regular_price:regular,sale_price:sale,effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,pgJson({cdw_parsed_id:parsedId,collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
    } else {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'CDW_PRICE_MISSING','medium','CDW dataset returned no usable price; product preserved but offer promotion withheld',$4)`,
    ...
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.cdw_product_images(cdw_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.cdw_product_categories(cdw_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
    for (const [i, spec] of r.variant_attributes.entries()) {
      ...
      if (!name) continue;
      await c.query(`INSERT INTO retail.cdw_product_specifications(cdw_parsed_id,specification_rank,specification_name,specification_value) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,name,value]);
    }
