...
export function availability(v:string|null|undefined){const x=(v??'').toLowerCase();if(/out.?of.?stock|unavailable|not available/.test(x))return 'out_of_stock';if(/in.?stock|available/.test(x))return 'in_stock';return 'unknown';}
export function isLenovoUrl(v:string){try{const u=new URL(v);return u.protocol==='https:'&&(u.hostname==='lenovo.com'||u.hostname.endsWith('.lenovo.com'));}catch{return false;}}
