...
import type { LenovoRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-lenovo-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='lenovo'`);
    if (!p.rowCount) throw new Error("Lenovo platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'lenovo_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, { dataset_id: config.LENOVO_DATASET_ID, seed_urls: urls }]
  );
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,next_retry_at)
     VALUES($1,$2,'lenovo',$3,$4,$5,$6,$7,now()+interval '15 minutes')
    ...
    [platformId,runId,hash,raw,code,message.slice(0,8000),attempt]
  );
}
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.lenovo_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
     VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(content_hash) DO NOTHING`,
    [runId,platformId,url,zone,contentType,status,sha256(body),body,stableJson(meta)]
  );
}

function isTargetScope(r:LenovoRecord): boolean {
  const text=[r.title,r.product_category,...r.category_tree.map(x=>x.name)].filter(Boolean).join(' ').toLowerCase();
  return /(lenovo|thinkpad|thinkbook|ideapad|yoga|legion|loq|thinkcentre|thinkstation|computer|laptop|notebook|desktop|workstation|monitor|dock|docking|keyboard|mouse|headset|webcam|storage|ssd|hard drive|accessor|gaming|server|case|bag|sleeve|charger|adapter)/i.test(text);
}

export async function ingestRecord(platformId:string,runId:string,r:LenovoRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    const product=await c.query(
      ...
      [platformId,r.item_id,r.url,r.title,r.brand,r.mpn,r.variant_id,r.product_category,r.image_url,stableJson({gtin:r.gtin,group_id:r.group_id,availability_date:r.availability_date})]
    );
    ...
    if (!scopeMatch) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'LENOVO_COLLECTION_SCOPE_MISMATCH','high','Lenovo laptop/gaming deal seed returned a product outside the governed Lenovo computing and accessories scope',$4)`,
        [platformId,productId,runId,stableJson({item_id:r.item_id,url:r.url,title:r.title,product_category:r.product_category,raw_capture_id:rawId})]
      );
    }
    if (priceInversion) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'LENOVO_PRICE_FIELD_INVERSION','high','Lenovo sale_price exceeded regular price; lowest valid observed price selected',$4)`,
        [platformId,productId,runId,stableJson({item_id:r.item_id,url:r.url,regular_price:regular,sale_price:sale,effective_price:effective,raw_capture_id:rawId})]
      );
    }
    if (regular !== null && sale !== null && Math.abs(regular - sale) < 0.005) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'LENOVO_DEAL_PAGE_NO_DISCOUNT','medium','Lenovo deal-page result has no observed price markdown; preserve evidence but do not assume discount qualification',$4)`,
        [platformId,productId,runId,stableJson({item_id:r.item_id,url:r.url,regular_price:regular,sale_price:sale,raw_capture_id:rawId})]
      );
    }
    ...
    await c.query(
      ...
      [productId,platformId,av,av==='in_stock',rawId,stableJson({raw_availability:r.availability,availability_date:r.availability_date})]
    );
    ...
    if (effective !== null) {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,stableJson({source:'lenovo',effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,stableJson({lenovo_parsed_id:parsedId,regular_price:regular,sale_price:sale,effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,stableJson({lenovo_parsed_id:parsedId,collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
    } else {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'LENOVO_PRICE_MISSING','medium','Lenovo dataset returned no usable price; product preserved but offer promotion withheld',$4)`,
        [platformId,productId,runId,stableJson({item_id:r.item_id,url:r.url,price:r.price,sale_price:r.sale_price,raw_capture_id:rawId})]
      );
    }

    const imgs=[r.image_url,...r.additional_image_urls].filter((x):x is string=>!!x);
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.lenovo_product_images(lenovo_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.lenovo_product_categories(lenovo_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
