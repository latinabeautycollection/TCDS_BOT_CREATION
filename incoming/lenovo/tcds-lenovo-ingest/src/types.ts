...
export const LenovoRecordSchema=z.object({
 url:z.string().url().refine(v=>new URL(v).hostname.toLowerCase().endsWith('lenovo.com'),'Product URL must belong to lenovo.com'),
 ...
 image_url:z.string().url().nullable().optional(),additional_image_urls:na(z.string().url()),additional_video_urls:na(z.string().url()),price:nn,sale_price:nn,availability:ns,availability_date:ns,
...
export type LenovoRecord=z.infer<typeof LenovoRecordSchema>;
