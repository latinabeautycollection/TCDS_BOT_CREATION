...
export async function triggerDataset(urls:string[]):Promise<string>{
 const endpoint=`https://api.brightdata.com/datasets/v3/trigger?dataset_id=${encodeURIComponent(config.HARBORFREIGHT_DATASET_ID)}&notify=false&include_errors=true&type=discover_new&discover_by=category_url&limit_per_input=${config.HARBORFREIGHT_LIMIT_PER_INPUT}`;
 const r=await fetchWithRetry(endpoint,{method:'POST',headers,body:JSON.stringify({input:urls.map(url=>({url})),limit_per_input:config.HARBORFREIGHT_LIMIT_PER_INPUT})});
