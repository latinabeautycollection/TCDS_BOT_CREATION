...
import type { HARBORFREIGHTRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-harborfreight-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='harborfreight'`);
    if (!p.rowCount) throw new Error("Harbor Freight platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'harborfreight_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, pgJson({ dataset_id: config.HARBORFREIGHT_DATASET_ID, seed_urls: urls })]
  );
  return r.rows[0].id as string;
}

export async function updateRunRequestTotal(id:string,total:number,snapshotId:string) {
  await pool.query(
    `UPDATE retail.collection_runs
     SET total_requested=$2,run_metadata=coalesce(run_metadata,'{}'::jsonb)||$3::jsonb
     WHERE id=$1`,
    [id,total,pgJson({snapshot_id:snapshotId,returned_rows:total})]
  );
}
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,next_retry_at)
     VALUES($1,$2,'harborfreight',$3,$4,$5,$6,$7,now()+interval '15 minutes')
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.harborfreight_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
...
function isTargetScope(r:HARBORFREIGHTRecord): boolean {
  const text=[r.title,r.product_category,...r.category_tree.map(x=>x.name)].filter(Boolean).join(' ').toLowerCase();
  return /(tool|drill|driver|impact|saw|grinder|sander|wrench|socket|ratchet|diagnostic|scanner|automotive|battery|charger|jump starter|inverter|compressor|generator|welder|shop equipment|hand tools|power tools)/i.test(text);
}

export async function ingestRecord(platformId:string,runId:string,r:HARBORFREIGHTRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    const product=await c.query(
      ...
      [platformId,r.item_id,r.url,r.title,r.brand,r.brand,r.mpn,r.variant_id,r.product_category,r.image_url,pgJson({gtin:r.gtin,group_id:r.group_id,availability_date:r.availability_date,source_brand:r.brand})]
    );
    ...
    if (!scopeMatch) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'HARBORFREIGHT_COLLECTION_SCOPE_MISMATCH','high','Harbor Freight electronics, laptops, and portable power-tools seed returned a product outside the governed TCDS scope',$4)`,
    ...
    if (av==='out_of_stock') {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'HARBORFREIGHT_OUT_OF_STOCK','low','Harbor Freight product is currently out of stock; evidence retained and offer marked unavailable',$4)`,
    ...
    if (priceInversion) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'HARBORFREIGHT_PRICE_FIELD_INVERSION','high','Harbor Freight sale_price exceeded regular price; lowest valid observed price selected',$4)`,
    ...
    if (regular !== null && (sale === null || Math.abs(regular - sale) < 0.005)) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'HARBORFREIGHT_SEED_RESULT_NO_DISCOUNT','medium','Harbor Freight category result has no observed markdown; preserve evidence but do not assume clearance or deal qualification',$4)`,
    ...
    if (effective !== null) {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,pgJson({source:'harborfreight',effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,pgJson({harborfreight_parsed_id:parsedId,regular_price:regular,sale_price:sale,effective_price_rule:'lowest_valid_observed_price',collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,pgJson({harborfreight_parsed_id:parsedId,collection_scope_match:scopeMatch,price_field_inversion:priceInversion})]
      );
    } else {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'HARBORFREIGHT_PRICE_MISSING','medium','Harbor Freight dataset returned no usable price; product preserved but offer promotion withheld',$4)`,
        [platformId,productId,runId,pgJson({item_id:r.item_id,url:r.url,price:r.price,sale_price:r.sale_price,raw_capture_id:rawId})]
      );
    }

    const imgs=[r.image_url,...r.additional_image_urls].filter((x):x is string=>!!x).slice(0,config.HARBORFREIGHT_MAX_IMAGES_PER_PRODUCT);
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.harborfreight_product_images(harborfreight_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.harborfreight_product_categories(harborfreight_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
    for (const [i, spec] of r.variant_attributes.entries()) {
      ...
      if (!name) continue;
      await c.query(`INSERT INTO retail.harborfreight_product_specifications(harborfreight_parsed_id,specification_rank,specification_name,specification_value) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,name,value]);
    }
