...
export class BrightDataClient {
  ...
  async trigger(input: Array<{url: string}>): Promise<{snapshotId: string; providerResponse: unknown}> {
    ...
    const response = await this.request(
      ...
      { method: "POST", body: JSON.stringify({ input, limit_per_input: this.cfg.BRIGHT_DATA_LIMIT_PER_INPUT }) },
      [200, 201, 202]
    );
  ...
  async fetchWebUnlockerHtml(targetUrl: string): Promise<string> {
    if (!this.cfg.BRIGHT_DATA_WEB_UNLOCKER_ZONE) {
      throw new Error("BRIGHT_DATA_WEB_UNLOCKER_ZONE is not configured");
    }
    const response = await this.request(
      `${this.cfg.BRIGHT_DATA_BASE_URL}/request`,
      {
        method: "POST",
        body: JSON.stringify({
          zone: this.cfg.BRIGHT_DATA_WEB_UNLOCKER_ZONE,
          url: targetUrl,
          format: "raw"
        })
      },
