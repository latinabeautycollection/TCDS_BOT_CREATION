...
import { BjsRecordSchema } from "./types.js";
...
async function claimNext() {
  ...
  try {
    ...
    const result = await client.query(`
      SELECT id, platform_id, collection_run_id, raw_payload, attempt_count
      FROM retail.ingest_dead_letters
      WHERE source_platform = 'bjs'
    ...
    if (row) {
      await client.query(
        `UPDATE retail.ingest_dead_letters SET status='retrying', last_failed_at=now() WHERE id=$1`,
        [row.id]
      );
    }
...
async function main() {
  for (let processed = 0; processed < 100; processed++) {
    ...
    if (!row) break;
    try {
      const record = BjsRecordSchema.parse(row.raw_payload);
