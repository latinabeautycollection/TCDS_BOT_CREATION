...
export class CanonicalDatabase {
  ...
  constructor(private readonly cfg: AppConfig) {
    this.pool = new Pool({
      ...
      application_name: "erip-bhphoto-retail-ingest"
    });
  }
  ...
  async health(): Promise<unknown> {
    return (await this.pool.query("select now() now, current_database() database")).rows[0];
  }

  async lock(): Promise<boolean> {
    const result = await this.pool.query(
      "select pg_try_advisory_lock(hashtextextended($1, 0)) locked",
      ["bhphotovideo:category-ingest"]
    );
  ...
  async startRun(requested: number, metadata: Record<string, unknown>): Promise<string> {
    ...
    `, [this.cfg.ERIP_WORKER_ID, JSON.stringify({ ...metadata, requested_rows: requested })]);
    return result.rows[0].run_id as string;
  }

  async ingest(runId: string, rows: unknown[], snapshotId: string | null): Promise<Counts> {
    const platform = await this.pool.query(
      "select id from retail.retail_platforms where platform_code='bhphotovideo'"
    );
    const platformId = platform.rows[0]?.id as string | undefined;
    if (!platformId) throw new Error("B&H platform is not registered; apply migration 025 first");
    ...
    for (const raw of rows) {
      ...
      const key = text(row.item_id) ?? url;
      if (!key || !url || !text(row.title)) {
        counts.failed++;
        await this.deadLetter(platformId, runId, row, "INCOMPLETE_PRODUCT", "Missing item_id, URL, or title", "pending");
        continue;
      }
      ...
      try {
        ...
        const capture = await client.query(`
          select retail.ingest_raw_product_capture(
            'bhphotovideo', $1::uuid, $2::text, $3::text,
            $4::text, $5::text, $6::text, $7::jsonb,
            'brightdata_bhphoto_v1', $8::jsonb
          ) as id
        `, [
        ...
        const parsed = await client.query(
          "select retail.ingest_brightdata_bhphoto_product($1::uuid,$2::jsonb) as id",
          [capture.rows[0].id, JSON.stringify(row)]
        );
        await client.query(
          "select retail.promote_bhphoto_parsed_product($1::uuid)",
          [parsed.rows[0].id]
        );
        ...
        counts.failed++;
        await this.deadLetter(
          platformId, runId, row, "BHPHOTO_INGEST_FAILED",
  ...
  private async deadLetter(
    ...
    await this.pool.query(`
      insert into retail.ingest_dead_letters(
        ...
        raw_payload, error_code, error_message, status, next_retry_at,
        resolution_notes
      ) values($1::uuid,$2::uuid,'bhphotovideo',$3,$4::jsonb,$5,$6,$7,null,$8)
