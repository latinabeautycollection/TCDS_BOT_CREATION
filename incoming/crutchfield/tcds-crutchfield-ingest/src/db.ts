...
import type { CrutchfieldRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-crutchfield-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='crutchfield'`);
    if (!p.rowCount) throw new Error("Crutchfield platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'crutchfield_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, { dataset_id: config.CRUTCHFIELD_DATASET_ID, seed_urls: urls }]
  );
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,next_retry_at)
     VALUES($1,$2,'crutchfield',$3,$4,$5,$6,$7,now()+interval '15 minutes')
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.crutchfield_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
...
export async function ingestRecord(platformId:string,runId:string,r:CrutchfieldRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    if (effective !== null) {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,{source:'crutchfield'}]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,{crutchfield_parsed_id:parsedId,regular_price:regular,sale_price:sale}]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,{crutchfield_parsed_id:parsedId}]
      );
    } else {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'CRUTCHFIELD_PRICE_MISSING','medium','Crutchfield dataset returned no usable price; product preserved but offer promotion withheld',$4)`,
    ...
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.crutchfield_product_images(crutchfield_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.crutchfield_product_categories(crutchfield_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
