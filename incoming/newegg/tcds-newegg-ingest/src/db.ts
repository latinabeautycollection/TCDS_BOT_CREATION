...
import type { NeweggRecord } from './types.js';
...
export const pool = new Pool({
  ...
  application_name: 'tcds-newegg-ingest'
});
...
export async function platformAndConfig() {
  ...
  try {
    await c.query('BEGIN');
    const p = await c.query(`SELECT id FROM retail.retail_platforms WHERE platform_code='newegg'`);
    if (!p.rowCount) throw new Error("Newegg platform missing; run migration");
...
export async function createRun(platformId:string, configId:string|undefined, runKey:string, urls:string[]) {
  const r = await pool.query(
    `INSERT INTO retail.collection_runs(platform_id,config_id,run_key,status,started_at,requested_by,total_requested,run_metadata)
     VALUES($1,$2,$3,'running',now(),'newegg_worker',$4,$5) RETURNING id`,
    [platformId, configId ?? null, runKey, urls.length, { dataset_id: config.NEWEGG_DATASET_ID, seed_urls: urls }]
  );
...
export async function deadLetter(platformId:string,runId:string|null,payload:unknown,code:string,message:string,retryable=true,attempt=1) {
  ...
  await pool.query(
    `INSERT INTO retail.ingest_dead_letters(platform_id,collection_run_id,source_platform,payload_hash,raw_payload,error_code,error_message,attempt_count,status,next_retry_at,resolution_notes)
     VALUES($1,$2,'newegg',$3,$4,$5,$6,$7,$8,CASE WHEN $8='abandoned' THEN NULL ELSE now()+interval '15 minutes' END,$9)
    ...
    [platformId,runId,hash,payload,code,message.slice(0,8000),attempt,retryable?'pending':'abandoned',retryable?null:'Non-retryable provider or public-price error']
  );
}
...
export async function saveEvidence(platformId:string,runId:string,url:string,zone:string,body:string,contentType:string,status:number,meta:unknown) {
  await pool.query(
    `INSERT INTO retail.newegg_unlocker_evidence(collection_run_id,platform_id,target_url,unlocker_zone,content_type,response_status,content_hash,response_body,evidence_metadata)
...
export async function ingestRecord(platformId:string,runId:string,r:NeweggRecord) {
  const c=await pool.connect();
  ...
  try {
    ...
    const product=await c.query(
      ...
      [platformId,productKey,r.url,r.title,r.brand,r.mpn,r.variant_id,r.product_category,r.image_url,{gtin:r.gtin,group_id:r.group_id,item_id:r.item_id,availability_date:r.availability_date}]
    );
...

    {
      await c.query(
        ...
        [productId,platformId,regular,sale,effective,rawId,{source:'newegg',price_field_inversion:priceFieldInversion,effective_price_rule:'lowest_valid_observed_price'}]
      );
      ...
      const offer=await c.query(
        ...
        [productId,platformId,effective,av,r.url,rawId,offerHash,{newegg_parsed_id:parsedId,regular_price:regular,sale_price:sale,price_field_inversion:priceFieldInversion,effective_price_rule:'lowest_valid_observed_price'}]
      );
      await c.query(
        ...
        [productId,platformId,offer.rows[0].id,effective,av,r.url,{newegg_parsed_id:parsedId,price_field_inversion:priceFieldInversion}]
      );
    }
    ...
    if (priceFieldInversion) {
      await c.query(
        `INSERT INTO retail.data_quality_events(platform_id,retail_product_id,collection_run_id,event_code,severity,event_message,event_json)
         VALUES($1,$2,$3,'NEWEGG_PRICE_FIELD_INVERSION','high','Newegg sale_price exceeded price; lowest valid observed price used and anomaly preserved for review',$4)`,
    ...
    for(const [i,u] of [...new Set(imgs)].entries())
      await c.query(`INSERT INTO retail.newegg_product_images(newegg_parsed_id,image_url,image_rank,is_main) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,u,i+1,i===0]);
    for(const [i,x] of r.category_tree.entries())
      await c.query(`INSERT INTO retail.newegg_product_categories(newegg_parsed_id,category_rank,category_name,category_url) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING`,[parsedId,i+1,x.name,x.url??null]);
