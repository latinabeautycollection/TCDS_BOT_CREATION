...
import { LowesRecordSchema } from "./types.js";
import { platformAndConfig, createRun, finishRun, deadLetter, ingestRecord, pool } from "./db.js";
import { isLowesUrl, sha256, stableJson } from "./util.js";
import { unlockLowesPage } from "./unlocker.js";
import { discoverLowesProducts, findUnreturnedLowesInputs } from "./discovery.js";
...
async function main() {
  const badUrl = config.seedUrls.find(url => !isLowesUrl(url));
  if (badUrl) throw new Error(`Rejected non-Lowe's seed URL: ${badUrl}`);

  const discovered = await discoverLowesProducts(
    config.seedUrls,
    async url => (await unlockLowesPage(url)).body,
    config.LOWES_LIMIT_PER_INPUT
  );
  const urls = discovered.map(product => product.url);
  if (!urls.length) throw new Error("LOWES_DISCOVERY_EMPTY");

  const { platformId, configId } = await platformAndConfig();
  const runKey = `lowes:${new Date().toISOString()}:${sha256(urls.join("|")).slice(0, 12)}`;
  const stats = { collected: 0, failed: 0, skipped: 0 };
  let runId = "";

  try {
    runId = await createRun(platformId, configId, runKey, urls);
    log("info", "lowes_run_started", { runId, seeds: config.seedUrls, discovered: urls.length });
    ...
    if (!rows.length) throw new Error("LOWES_SNAPSHOT_EMPTY");

    const unreturned = findUnreturnedLowesInputs(urls, rows);
    stats.skipped += unreturned.length;
    if (unreturned.length) {
      log("warn", "lowes_provider_inputs_unreturned", {
        runId,
        snapshotId,
    ...
    await mapLimit(rows, config.INGEST_CONCURRENCY, async raw => {
      ...
      const parsed = LowesRecordSchema.safeParse(raw);
      if (!parsed.success) {
        stats.failed++;
    ...
    const reason = status === "partial"
      ? `${stats.skipped} provider inputs unresolved; ${stats.failed} ingestion failures`
      : undefined;
    await finishRun(runId, status, stats, reason);
    log("info", "lowes_run_completed", { runId, ...stats, status });
  } catch (error) {
    if (runId) await finishRun(runId, "failed", stats, String(error));
    log("error", "lowes_run_failed", { runId, error: String(error), ...stats });
    throw error;
  } finally {
    await pool.end();
  }
