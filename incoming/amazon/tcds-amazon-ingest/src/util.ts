...
export const uniqueRunKey=(prefix:string,scope:string)=>`${prefix}:${new Date().toISOString()}:${sha256(scope).slice(0,12)}:${crypto.randomUUID()}`;
...
export function isAmazonUrl(v:string){try{const u=new URL(v);return u.protocol==='https:'&&(u.hostname==='amazon.com'||u.hostname.endsWith('.amazon.com'));}catch{return false;}}
