...
export async function attestableSha(authorityType:'file'|'package_tree',rootOrFile:string){
  if(authorityType==='file') return (await shaFile(rootOrFile));
  return (await shaPackageTree(rootOrFile)).sha256;
}
