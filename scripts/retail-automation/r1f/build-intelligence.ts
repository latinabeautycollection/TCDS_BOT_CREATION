...
const actor=process.env.R1F_ACTOR_NAME||'R1F V2 Intelligence Worker';

if(!policyId){
  throw new Error('usage: tsx build-intelligence.ts <policy_uuid> [window_end_iso]');
}

async function main(){
  const windowEnd=windowEndArg||new Date().toISOString();
  const run=await startPersistentRun(
    pool,'RETAIL_R1F_BUILD_INTELLIGENCE','worker',
    'r1f-intelligence-worker',actor,
    'retail.r1f_intelligence_snapshots'
  );
  try{
    ...
    console.log(JSON.stringify({
      event:'r1f_v2_intelligence_built',
      policyId,windowEnd,
