...
const code=codeArg||'r1f_green_tier1';
const version=versionArg||'2';
const actor=actorArg||process.env.R1F_CERTIFIER||'R1F Certification Policy Authority';
...
async function main(){
  ...
  const run=await startPersistentRun(
    ...
    'retail.r1f_certification_policies'
  );
  ...
  try{
    ...
    const r=await c.query(`
      insert into retail.r1f_certification_policies(
    ...
    await c.query(`
      select retail.r1f_certify_certification_policy(
        $1,$2,$3,$4
      )
    ...
    console.log(JSON.stringify({
      event:'r1f_certification_policy_certified',
