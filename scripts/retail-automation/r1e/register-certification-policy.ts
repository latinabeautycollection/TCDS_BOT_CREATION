...
const [policyFile,codeArg,versionArg,actorArg]=process.argv.slice(2);
const code=codeArg||'r1e_green_tier1';
const version=versionArg||'2';
const actor=actorArg||process.env.R1E_CERTIFIER||'R1E Policy Authority';

if(!policyFile){
  throw new Error('usage: tsx register-certification-policy.ts <policy.json> [code] [version] [actor]');
}

async function main(){
  const policy=JSON.parse(await readFile(policyFile,'utf8'));
  const run=await startPersistentRun(
    pool,'RETAIL_R1E_V2_POLICY_REGISTER','user',actor,actor,
    'retail.r1e_certification_policies'
  );
  const c=await pool.connect();
  try{
    await c.query('begin');
    const r=await c.query(`
      insert into retail.r1e_certification_policies(
    ...
    await c.query(`
      select retail.r1e_certify_policy($1,$2,$3,$4)
    ...
    console.log(JSON.stringify({
      event:'r1e_v2_certification_policy_certified',
